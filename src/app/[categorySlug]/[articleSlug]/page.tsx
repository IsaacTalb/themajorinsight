...
export default async function ArticlePage({ params }: Props) {
  ...
  return <main>
    <article className="site-container py-10 md:py-16">
      ...
      <nav aria-label="Breadcrumb" className="flex flex-wrap gap-2 text-xs text-muted"><Link href="/">Home</Link><span>/</span><Link href={`/${categorySlug}`}>{article.categoryName}</Link><span>/</span><span aria-current="page">{article.type}</span></nav>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[8rem_minmax(0,46rem)_1fr] lg:gap-10">
        ...
        <div>
          ...
          <div id="analysis" className="prose-editorial font-editorial text-xl leading-[1.78] text-charcoal">{article.body.map(p => <p key={p}>{p}</p>)}</div>
