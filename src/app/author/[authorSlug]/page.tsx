...
export function generateStaticParams() { return [...new Set(articles.map(a => a.author.slug))].map(authorSlug => ({ authorSlug })); }
export default async function AuthorPage({ params }: { params: Promise<{ authorSlug: string }> }) {
  const { authorSlug } = await params; const work = articles.filter(a => a.author.slug === authorSlug); if (!work.length) notFound(); const author = work[0].author;
