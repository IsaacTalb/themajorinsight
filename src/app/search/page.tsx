...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const q = (await searchParams).q?.trim() ?? ""; const query = q.toLowerCase(); const articles = await getLatestInsights(50); const results = query ? articles.filter(a => [a.title,a.excerpt,a.categoryName,...a.tags].join(" ").toLowerCase().includes(query)) : [];
